...
const ProductionCalculator = () => {
  ...
  const calculateValues = React.useCallback(() => {
    ...
    const hoursArr = [];
    const totalHours = Math.ceil(runtime);
    const boxesPerHour = actualBoxes / runtime;
    
    for (let i = 0; i < totalHours; i++) {
      const hour = i + 1;
      const startDecimal = startTimeDecimal + i;
      const startHourStr = formatTimeString(startDecimal);
      const endHourStr = formatTimeString(startDecimal + 1);
      
      hoursArr.push({
        name: `Hour ${hour}`,
        timeRange: `${startHourStr}-${endHourStr}`,
        actualBoxes: Math.round(boxesPerHour),
        targetBoxes: Math.round(nomYield),
        efficiency: Math.min(Math.round((boxesPerHour / nomYield) * 100), 100)
      });
    }
    
    setHourlyData(hoursArr);
  ...
  return (
    <div className={`p-4 min-h-screen ${mainBgColor} transition-all duration-300`}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className={`${cardBgColor} p-6 rounded-lg shadow-lg transition-all duration-300 mb-6`}>
          <h2 className={`text-xl font-semibold mb-4 ${headingColor}`}>Hourly Production Tracking</h2>
              ...
              >
                ...
                <Tooltip 
                  ...
                  formatter={(value, name) => {
                    ...
                    return [value, name === 'actualBoxes' ? 'Actual Boxes' : 'Target Boxes'];
                  }}
                />
                <Legend />
                <Line type="monotone" dataKey="actualBoxes" stroke="#10B981" strokeWidth={2} activeDot={{ r: 8 }} name="Actual Boxes" />
                <Line type="monotone" dataKey="targetBoxes" stroke="#3B82F6" strokeWidth={2} name="Target Boxes" />
